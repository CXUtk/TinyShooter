...
Author: DXTST
****************************************************************
1. Using "C" style to constructing games in SDL?
	That might be a good idea.
	For me, "C" is an exceedingly elegent language.
		The "C" language is significantly easier. It is concise and basic. On the other hand, C++ has a large amount of advanced 
	features and almost the same (or even more) flexibility and control over primerary computer layer. However, the complexity of
	"C++" is really precipitous when I tried to get deeper understanding.

2. I made this game basically for get know about the merge type of C and C++, as well for increasing my ability to manage a 
	large game. I believe it would be a brilliant experience.

****************************************************************
PS: I'd prefer DirectX instead. (Run away
*/
#include <iostream>
#include <ctime>
#include <fstream>
...
void Game_main()
{
	...
	while (!quit)
	{
		...
		if (!isMenu) 
		{
			...
			settingButton.Render(g_GameRenderer);
...
bool Game_init()
{
	...
	{
		//Create window
		g_Window = SDL_CreateWindow("AI Test", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
		if (g_Window == nullptr)
		{
		...
		else
		{
			...
			else
			{
				...
				SDL_SetRenderDrawColor(g_GameRenderer, 255, 255, 255, 255);
...
bool Game_loadContent()
{
	...
	settingButton.SetTexture(g_GameRenderer, g_Font, "Settings");
	exitButton.SetTexture(g_GameRenderer, g_Font, "Quit Game");
	player.playerTex.LoadTexFromFile(g_GameRenderer, "Asset/Player.png");
	g_MenuTexture.LoadTexFromFile(g_GameRenderer, "Asset/MenuImg.png");
	...
	addTextureToArray<Texture2D>(EnemyTexture, "Asset/Enemy/enemy_0.png", g_npcID);
...
{
	//Setting button
	settingButton.posBox = { SCREEN_WIDTH/2 - 40, SCREEN_HEIGHT - 150 , 80, 30 };
	settingButton.SetButtonEvent([](const SDL_Event& e, SDL_Rect& rect, Texture2D& tex) {
		if (e.type == SDL_MOUSEMOTION || e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEBUTTONUP) {
			//Check if mouse is in button
			bool inside = true;
